...
getDynamicConstraints <- function(data, tStep=1, nSteps=60, 
    ...
    if (file.exists(data)) {
        ...
        if (exists(data) && is.function(get(data))) {
            ...
            cat('getDynamicConstraints: ERROR - ', data, '.R should contain\n',
        ...
        cat("\n\ngetDynamicConstraints: File", data, "could not be read!\n\n\n")
	return(NULL)
    }
    ...
    x <- dat['Time'][,1]	# Prepare a vector with the time values
    				# note that it may not exist!!! In that
                                # case we should just return dat as is.
    # We can
	...
	y <- dat[r][,1]		# Prepare a vector with reaction values
		...
		p <- coef(mdl)
		obsdata <- data.frame(
			x = seq(tStep, nSteps * tStep, by=tStep),
                        # x = out$Time,
			y = sigm(p, x)
		)
            if (r == biomass || 
	    ...
	    if (interp == 'linear') {
		    ...
		    approx(x, y, xout=seq(tStep, nSteps * tStep, by=tStep))
#		    approx(x, y, xout=out$Time)
		)
	    } else if (interp == 'spline') {
	        interdata <- data.frame(
		    spline(x, y, xout=seq(tStep, nSteps * tStep, by=tStep))
#		    spline(x, y, xout=out$Time)
		)
	    }
	}
...
getSubstrateDeltas <- function(data, tStep=1, nSteps=60) {
    if (data == '') {
    	return (NULL)
    }
    ...
    if (file.exists(data)) {
        ...
        if (exists(data) && is.function(get(data))) {
            ...
            cat('getSubstrateDeltas: ERROR - ', data, '.R should contain\n',
                '    a function ', data, '(model, concentrations, fluxes, step)', sep='')
            return(NULL)
        }
        ### JR ### NEEDS TO BE TESTED
    } else {
        cat("\n\ngetSubstrateDeltas: File", data, "does not exist!\n\n\n")
	return(NULL)
    }
    
    # provide feedback
    cat("\ngetSubstrateDeltas: processing", data, "\n\n")
    print(dat)
    
    # get list of exchange reaction names
    reactions <- names(dat)
    reactions <- reactions[ -which(reactions %in% 'Time') ]
...
readSubstrate <- function(substrate="") 
{
    ...
    for ( i in 1:dim(subs)[1] ) {
        ...
        if (mg_per_l >= 0) {
            ...
            #print(paste("Excluding", exch, "because mg/l < 0"))
...
dfba <- function(model, dfbaProblem, outDir=".", verbose=0) {
    ...
    outConcentrations <- paste (dfbaDir, '/', modelName, "_concs.txt", sep="")
    outFluxes <- paste (dfbaDir, '/', modelName, "_fluxes.txt", sep="")
    outBiomass <- paste (dfbaDir, '/', modelName, "_biomass.txt", sep="")
...
adfba <- function(model, adfbaProblem, outDir=".", verbose=0) {
    ...
    outConcentrations <- paste (adfbaDir, '/', modelName, "_concs.txt", sep="")
    outFluxes <- paste (adfbaDir, '/', modelName, "_fluxes.txt", sep="")
    outBiomass <- paste (adfbaDir, '/', modelName, "_biomass.txt", sep="")
...
# plot agarase secretion
#png(paste(adfbaDir, '/', "adfba_SEC_DAG.png", sep=''), width=1000, height=1000)
#lplot(af_sol, plotRxns="EX_dag(e)")
#dev.off()

    # Create typical COBRA-Style DFBA plot
    #png(pngDFBAplot)
    cat("Summary plots: splines\n")
    x11()
    ...
    # instead of splines
    #png(pngDFBAplot)
    cat("Summary plots: lines\n")
    x11()
    #lplotOptSolDFBA(af_sol, plotRxns=plotRxns);
    lplot(af_sol, plotRxns=plotRxns)
...
adynfba <- function() {
    ...
    defst <- 72			# default number of time steps
    ...
    defdeltas <- ""		# default substrate deltas at each time step
	...
	#-h / --help is added by default
        ...
        make_option(c("-l", "--deltas"), 
            type="character", 
            default=defdeltas,
            help=paste("incremental substrate changes at each time point 
    ...
    ts <- opt$timestep / 60		# convert to hours which is our time unit
    ns <- opt$steps			# number of steps
    inoculum <- opt$inoculum		# initial Biomass
    biomass <- opt$biomass		# name of biomass function
    rateChanges <- opt$exch		# "" if not specified
    subsDeltas <- opt$deltas		# "" if not specified
    ...
    if (rateChanges != "")
        outName <- paste(outName, "+", rateChanges, sep="")
    if (substrateName != "")
        outName <- paste(outName, "@", substrateName, sep="")
    if (subsDeltas != "")
        outName <- paste(outName, "+", subsDeltas, sep="")

    outName <- paste(outName, ":", ts, "hx", ns, sep='')
    ...
    if ((doDFBA == TRUE) || (doADFBA == TRUE)) {
        if (substrateName == "") {
            if (file.exists('setUpSubstrate.R')) {
                # This file should contain a setUpSubstrate() function
                source('setUpSubstrate.R')
	    }
            # provide a default substrate
            if (exists("setUpSubstrate") && is.function(get("setUpSubstrate"))) {
    ...
    tryCatch( 
    {
	    ...
	    if (rateChanges != "") {
		# This will check if the file is a text or an R file
                # and return either a data object or a function
                dynamicConstraints <- getDynamicConstraints(rateChanges,
		    timeStep, nSteps)
	    }
            else {
            	dynamicConstraints <- NULL
	    ...
	    if (subsDeltas != "") {
		# check file type and return either a table or a function
	        substrateDeltas <- getSubstrateDeltas(subsDeltas,
		    timeStep, nSteps)
	    ...
	    plotRxns <- c(medium$substrateRxns);
            ...
            # use one of these to coerce a given method
            #method <- "FBA"
            #method <- "lpFBA"
            #method <- "directFBA"
            #method <- "MTF"
	    ...
	    adfbaProblem <- list(
		    ...
		    biomassRxn=biomass,
		    dynamicConstraints=dynamicConstraints,
		    nutrientChanges=substrateDeltas,
                    method=method
		    )
