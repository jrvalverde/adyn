...
getSubstrateDeltas <- function(data, tStep=1, nSteps=60) {
    ...
    if (file.exists(data)) {
        ...
        cat("\n\ngetSiubstrateDeltas: File", data, "does not exist!\n\n\n")
	return(NULL)
    }
...
adynfba <- function() {
    ...
    ratechange <- opt$exch		# "" if not specified
	...
	ext <- ""
        if (ratechange != "") { ext <- paste(ext, "+", ratechange, sep="") }
    ...
    tryCatch( 
    {
	    ...
	    if (ratechange != "") {
		# check if this is or there is a .R file and, if so,
                # source it and set dynamicConstraints to the filename
                # without .R (i.e. to a function defined in the file,
                # which should be named after the control function)
                # if (ratechange ends in '.R') { 
                #    isRfile=TRUE 
                #    rates=ratechange - '.R'
                # }
                # else if (file_exists(ratechange'.R') {
                #    isRfile=TRUE
                #    rates=ratechange
                #    ratechange=ratechange'.R'
                # }
                # if (isRfile) {
                #    source ratechange
                #    test if rates exits and its type
                #	if it is either a function or an array, use it
                #    if not, maybe we need to load it as an Rdata file
                #    if neither,
                #    proceed try to process it as a data file
                #    isRfile <- FALSE
                # }
                dynamicConstraints <- getDynamicConstraints(ratechange,
		    timeStep, nSteps)
	    }
            else {
            	dynamicConstraints <- NULL
