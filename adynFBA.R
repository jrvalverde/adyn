...
#SYBIL_SETTINGS('SOLVER', 'glpkAPI') # should work  
...
solver <- Sys.getenv("SOLVER", unset=NA)
if (is.na(solver)) {
    solver <- 'glpkAPI'		# by default we'll use GLPK
}
...
#	echo doing $i ; 
#	sbR --slave -e 'source("dynFBA.R")' --args `basename $i _react.tsv` $constrains $medium 60 ; 
#	echo $i done ; 
#    done
#
# for i in xxx*_react,tsv ; do sbR --slave -e 'source("dynFBA.R")' --args `basename $i _react.tsv` basename $contrains .tab` $medium 60 ; echo $i ; done
#
# for i in *.tab ; do sbR --slave -e 'source("dynFBA.R")' --args Slp_aml `basename $i .tab` NMMP 60 ; echo Slp_aml $i ; done
...
loadModel <- function(modelName) {
    ...
    if (model@mod_name == '') model@mod_name <- modelName
    return( model )
}
...
mtf <- function(model, outDir='.', verbose=0) {
    ...
    #print( lp_obj(fl) )
    nreact <- length(fl)	# should we use nreact<-length(fl)/2 ???
    for (i in 1:nreact) {
...
#
#	Performs a gene perturbation analysis for each of the genes specified
# using the lower and upper bounds indicated for the depending reactions
#
#	Can calculate results using 'fba', 'lmoma' or 'room'
#
simpleSingleGDA <- function(model, 
		gene=model@allGenes, 
		low.bnd=rep(0, length(gene)), 
		upp.bnd=rep(0, length(gene)), 
		method="lmoma", 
                outDir='.', verbose=0)
{
    modelName <- model@mod_name
    dir.create(outDir, showWarnings = FALSE)
    sgdaDir=paste(outDir, "/singleGeneDeletion", sep="")
    dir.create(sgdaDir, showWarnings = FALSE)
    tabSGDAbase <- paste(sgdaDir, "/", 
                           modelName, "_ko_",  
			   sep="");
    pngSGDAfile <- paste(sgdaDir, "/", 
	    modelName, '_ko_',
	    method, '.png', sep='')

#    gd <- geneDeletion(model, gene="G_lcl_HE798556_1_prot_CCH14388_1_3785", 
#    	algorithm='fba')
#    gd <- oneGeneDel(model, gene='G_lcl_HE798556_1_prot_CCH14388_1_3785', 
# 	lb=0, ub=0, algorithm='lmoma', checkId=TRUE)
#    gd <- oneGeneDel(model, gene='G_lcl_HE798556_1_prot_CCH14388_1_3785', 
# 	lb=0, ub=0, checkId=TRUE,
#	algorithm='room', solverParm=list(PRESOLVE=GLP_ON))

    cat('\n----------------------------------------')
    cat('\nNOTE: Computing mutants one-by one')
    cat('\n----------------------------------------\n')

    # take this out of the loop for efficiency
    mtfdist <- getFluxDist(optimizeProb(model, algorithm='mtf'))
    for (i in 1:length(gene)) {
        g=gene[i]
	if (is.numeric(g)) {
	    # if numerically indexed, let it be the corresponding gene name
	    g <- model@allGenes[gene]
	}
        l=low.bnd[i]
        u=upp.bnd[i]
	cat("-------------------------------------------------------------------------------\n")
        cat(paste("\nProcessing Gene no.", i, g, '\n'))
        tabSGDAfile <- paste(tabSGDAbase, g, 
		    '_lb=', l, '_ub=', u, '_',
		    method, '.txt', sep='')
        if (file.exists(tabSGDAfile)) {
            cat(paste('\n', tabSGDAfile, "already exists, skipping\n"))
            next
        }
        # method can be 'fba', 'mtf', 'lmoma' or 'room'
        # ROOM and MOMA can take a reference wt distribution
        # we'll use the MTF one
        if (method == 'room') {
            if (solver == 'glpkAPI') {
                # maximum time for searching for a solution (in ms)
                tmlim <- Sys.getenv('TM_LIM', unset=NA)
                if ((is.na(tmlim)) || (tmlim == 0)) tmlim <- 10000
                # room requires additional arguments with glpk
                gd <- oneGeneDel(model, gene=g, lb=l, ub=u,
		      algorithm='room', fld='all', 
                      wtflux=mtfdist, 
                      solverParm=list(TM_LIM=tmlim, PRESOLVE=GLP_ON))
            } else {
                gd <- oneGeneDel(model, gene=g, lb=l, ub=u,
		      algorithm='room', fld='all',
                      wtflux=mtfdist,
                      solverParm=list(TM_LIM=tmlim))
            }
        } else if (method == 'lmoma') {
            gd <- oneGeneDel(model, gene=gene, lb=low.bnd, ub=upp.bnd,
		  algorithm='lmoma', fld='all',
                  wtflux=mtfdist)
        } else if (method == 'mtf') {
            # we cannot run oneGeneDel wit algorithm='mtf' (why? CHECK --JR)
	    # but we can run optimizeProb with 'mtf' 
            gd <- optimizeProb(model, gene=g, lb=l, ub=u, algorithm='mtf')
        } else {
            gd <- oneGeneDel(model, gene=g, lb=l, ub=u,
		    algorithm=method, fld='all')
        }
        
        # Now that we have the solution, let us print out the result
        cat("\nGene ", g, " (lb=", l, ") (ub=", u, "):\n", sep='')
	cat("\nDeleted", g, "\n")
        if (method != "mtf") {
            # for MTF since we use optimizeProb, we do not have these fields
 	    # gd@hasEffect is a boolean array telling which genes have effect
	    if (gd@hasEffect) {
	        cat('\nReactions affected by gene', g, 'are\n')
	        # gd@fluxdels is a list of arrays
	        # each array is a list of reaction names affected by a gene in the list
	        reacts <- gd@fluxdels
	        for (j in reacts) {
		    cat (j, '\t', sep='')
	        }
	        ...
	        cat('\nGene', g, 'has no effect\n')
            }
        }
        # gd@lp_ok contains the exit code for each optimization
	# gd@lp_stat tells the final status of each calculation (5=ok)
	if (gd@lp_ok == 0 && gd@lp_stat == 5) {
	    # gd_lp_obj is a array with the computed obj value for each problem
	    cat('Value of the objective function (', gd@obj_func, 
	        '):', gd@lp_obj, '\n')
	} else {
	    cat('Calculations did not complete successfully (',
	        gd@lp_ok, ',', gd@lp_stat,
		')\n')
	    #next # we'll try to save results anyway
	}
	if (verbose > 0) {
	    cat('Recovering fluxes for', g, 'with', method, '\n')
            fl <- getFluxDist(gd)
	    # as an alternative use this which should be equivalent to 
            # getFLuxDist(gd)
            #fl <- gd@fluxdist@fluxes[gd@fldind]

	    nfluxes <- length(fl)
            nreact <- length(react_id(model))
	    cat(paste('\nSaving ', length(fl), ' fluxes to "', tabSGDAfile,'"\n', sep=""))
            if (nfluxes == nreact) {
                cat('    Writing file\n') 
                out <- cbind(react_id(model), fl)
                colnames(out) <- c("React", "flux")
                write.table(out, file=tabSGDAfile, 
                            quote=FALSE, row.names=FALSE, col.names=FALSE)
            } else {
                cat("    nfluxes (", nfluxes, ") not equal to nreact (", 
                    nreact, "), saving anyway,\n")
	        for (flux in 1:nfluxes) {
        	    # output as CSV to log file
        	    #	    fl[flux]))
        	    out <- paste(react_id(model)[flux], '	', 
    			        fl[flux])
        	    cat(out, file=tabSGDAfile, sep="\n", append=TRUE)
	        }
            }
        }
    }
...
#	algorithm='room', solverParm=list(PRESOLVE=GLP_ON))
#
# genes <- c('G_lcl_HE798556_1_prot_CCH14512_1_3909', 'G_lcl_HE798556_1_prot_CCH11026_1_423', 'G_lcl_HE798556_1_prot_CCH12371_1_1768', 'G_lcl_HE798556_1_prot_CCH14388_1_3785')
# genes <- c("G_lcl_HE798556_1_prot_CCH13289_1_2686", "G_lcl_HE798556_1_prot_CCH11818_1_1215", "G_lcl_HE798556_1_prot_CCH11225_1_622", "G_lcl_HE798556_1_prot_CCH11234_1_631")

    # method can be 'fba', 'mtf', 'lmoma' or 'room'
    if (method == 'room' || method == 'mtf') {
        # for now, ROOM and MTF do not work for more than one gene
        # so, we'll do them gene by gene by calling simpleSingleGDA
        
        return(simpleSingleGDA(model, gene, low.bnd, upp.bnd, method, 
                               outDir, verbose))
        ...
        if (solver == 'glpkAPI') {
            ...
            gd <- oneGeneDel(model, gene=gene, lb=low.bnd, ub=upp.bnd,
		  algorithm='room', fld='all',
                  wtflux=getFluxDist(optimizeProb(model, algorithm='mtf')), 
        ...
        # we cannot run oneGeneDel wit algorithm='mtf' (why? CHECK --JR)
	# but we can run optimizeProb with 'mtf' later 
        gd <- oneGeneDel(model, gene=gene, lb=low.bnd, ub=upp.bnd,
		algorithm='fba')
	# we don't ask for the fluxes since we'll not be able to use them
        # (they are 'fba', not 'mtf')
    } else {
        gd <- oneGeneDel(model, gene=gene, lb=low.bnd, ub=upp.bnd,
		algorithm=method, fld='all')
    }

    # make frequency plot of genes vs. impact
    cat("\nPlotting frequencies of impacts on objective to", pngSGDAfile, "\n")
    png(pngSGDAfile)
    print(plot(gd, width=1000, height=1000))
    dev.off()

    #
    cat("Gene Deletion Experiment coomputed using", gd@algorithm, "\n")
    cat("Solved", gd@num_of_prob, "problems using the", gd@solver, "solver with the", gd@method, "method\n")
    #
    ngenes <- length(gene)
    cat("Analyzed", ngenes, "genes\n")
    for (i in 1:ngenes) {
        # genes are the genes analyzed, either named or numerically indexed
	g <- gene[i]
	if (is.numeric(g)) {
	    # if numerically indexed, let it be the corresponding gene name
	    g <- model@allGenes[gene]
	...
	if (gd@hasEffect[i]) {
        ...
        # gd@lp_ok contains the exit code for each optimization
	# gd@lp_stat tells the final status of each calculation (5=ok)
	if (gd@lp_ok[i] == 0 && gd@lp_stat[i] == 5) {
	    # gd_lp_obj is a array with the computed obj value for each problem
	    cat('Value of the objective function (', gd@obj_func, 
	        '):', gd@lp_obj[i], '\n')
	} else {
	    cat('Calculations did not complete successfully (',
	        gd@lp_ok[i], ',', gd@lp_stat[i],
		')\n')
	    #next # we'll try to save results anyway
	}
	# gd can be subindexed, then it behaves as an array of solutions
	# we could get the fluxdist for each gene by  looking at its solution
	# fluxes <- fluxdist(gd)[i], getFLuxDist(gd)[, i] or 
        # gd@fluxdist@fluxes[i];
	# if we want to get them, we will need to recompute each one
	# specifically with optimizeProb
	# Soooo... let us recompute the fluxes (if needed) and save them
	if (verbose > 0) {
	    tabSGDAfile <- paste(tabSGDAbase, g, 
		    '_lb=', gd@chlb[i], '_ub=', gd@chub[i], '_',
		    method, '.txt', sep='')
            cat('Recovering fluxes for', g, 'with', method, '\n')
    	    if (method != 'mtf') {
                fl <- getFluxDist(gd)[, i]
                # this returns additional values for lmoma
                # (the wt and the mutant)
		### ALTERNATIVE ###
		# optsol@fluxdist@fluxes is a bidimestional matrix
                # with as many rows as reactions and one column for
                # each computed flow distribution (here for each
                # mutant. So, here, fluxes[i,] gets the values of
                # a given reaction in all mutants and fluxes[ ,i]
                # the values of all reactions in a given mutant.
                #flv <- gd@fluxdist@fluxes[ ,i]		# values
                # NOTES:
                # for MTF this gives an array 3 x nreact, where
                # set1 = fluxes, set2=backwardfluxes, set3=forwardfluxes
                # for lMOMA this gives and array 4 x nreact where
                # set1 = wtreffluxes, set2=fluxes, ...
                # for all, this gives an sparse array 
                #
                # so we need to mimic getFLuxDist() here: the following
                # code should give the same result as getFluxDist():
		#   gd@fluxdist@fluxes[gd@fldind,] == getFluxDist(gd)
                #fl <- gd@fluxdist@fluxes[gd@fldind, i]
                # we could move this out of the loop and just index here
                #   ... gdfluxes <- gd@fluxdist@fluxes[gd@fldind, ]
                #   fl <- gdfluxes[ ,i]
	    } else {
	        # For MTF we had to use FBA in oneGeneDel, so we have to
		# recalculate the MTF now.
		sol <- optimizeProb(model, gene=gene[i], 
                    lb=gd@chlb[i], ub=gd@chub[i], 
		    algorithm=method)	# method IS 'mtf'
		    # without wtobj, MTF uses react (derived from gene), lb 
		    # and ub, and the objective will be calculated using FBA
	    	fl <- getFluxDist(sol)
	    }
	    nfluxes <- length(fl)
            nreact <- length(react_id(model))
	    ...
	    # END SAVE Single Gene Deletion Analysis VALUES	
	}
    }
...
dfba <- function(model, dfbaProblem, outDir=".", verbose=0) {
    ...
    cat('\n------------------------------------------------------------------------\n')
    cat('\nDoing DFBA\n')
    # this is our enhanced local version for extra reporting
    #source('dynamicFBAplus.R')
    #df_sol <- dynamicFBAplus(model, substrateRxns=substrateRxns, 
...
### j		    dynamicConstraints=dynamicConstraints,
	...
	cat("Please, wait...\n")
	    ...
	    plotExchRxns1by1(df_sol, outDir=dfbaDir, prefix=modelName, inactive=TRUE)
	else
...
#        plotRxns, verbose, Slividans, file="Slividans.RData")
	...
	cat("Please, wait...\n")
	    ...
	    # This function saves images in files named by itself!!!
	    #	That is why it wants to know te model's name to use as a prefix
	    plotExchRxns1by1(af_sol, outDir=adfbaDir, prefix=modelName, inactive=TRUE)
	else
...
saveConcentrations <- function(dfba, file="dynConcs.tab")
{
    feats <- attributes(dfba)

    write.table(feats$concentrationMatrix, file=file, sep='\t')
                

}

saveFluxes <- function(dfba, file="dynFluxes.tab")
{
    feats <- attributes(dfba)

    write.table(feats$all_fluxes, file=file, sep='\t')
...
plotExchRxns1by1 <- function(dfs, outDir='.', prefix="TS_PLOT", inactive=FALSE) {
    times <- dfs@timeVec
    concs <- as.matrix(dfs@concentrationMatrix)
    # if we want to plot only non-zero ExcRxns
    # concs <- concs[apply(concs[, -1], 1, function(x) !all(x==0)),]
...
adynfba <- function() {
    ...
    defnutrients <- ""		# default growth medium
    defdeltas <- ""		# default substrate deltas per time step
    defko <- ""			# default knock-out genes to test
    defjoin <- 0		# default number of KO genes to combine
    defgpa <- "lmoma"		# default method for gene perturbation analysis
    verbose <- 0		
	...
	#-h / --help is added by default
        ...
        make_option(c("-j", "--join"), 
            type="integer", 
            default=defjoin,
            help=paste("Max. number of KO genes to combine in perturbation analysis
            [default '%default']", sep="")),

        make_option(c("-z", "--zap"), 
            type="character", 
            default=defko,
            help=paste("list of knock-out genes and bounds to perturbate (g\\t\\l\\tu\\n)
            if no file name is provided and -j is specified, then all genes will
            be analyzed setting their lower and upper bounds to zero (it can be
            either a list of gene names or a list of gene index numbers)
            [default '%default'] (only used with --join)", sep="")),

	make_option(c("-q", "--gpamethod"),
            type="character",
            default=defgpa,
            help=paste("method to use for gene perturbation analysis (GPA) 
            calculations, only used with -j and -z. Must be one of 'fba',
            'mtf', 'moma' (Minimization of Metabolic Adjustment), 'lmoma' 
            (linearized version of MOMA), or 'room' (Regulatory On/Off
            Minimization). [default '%default']", sep="")),
    ...
    join <- opt$join			# max number of KO genes to combine
    koGenesName <- opt$zap		# KO genes to zap and their L/U limits
    gpaMethod <- opt$gpamethod		# method to use for gene perturbation
    ...
    # Check if a Gene Perturbation Analysis has been requested
    if (opt$join > 0) { doGPA <- TRUE } else { doGPA <- FALSE }
    if (opt$join > 1) {
        cat("\nGENE PERTURBATION ANALYSIS requested for combinations of", join, "genes\n")
        cat("However, only 1 gene perturbation analysis is currently supported\n")
        cat("Setting the number of genes to combine to 1\n\nSorry\n\n")
	join <- 1
    }
    
    validGPAmethods <- c('fba', 'mtf', 'moma', 'lmoma', 'room')
    if (! gpaMethod %in% validGPAmethods) {
        cat("\nGPA METHOD requested", gpaMethod, "is not valid (see help)")
        gpaMethod <- defgpa
        cat("\nWe will be using", gpaMethod, "for Gene Perturbation Analysis\n")
    }
    ...
    #koGenesName <- normalizePath(koGenesName)
    ...
    tryCatch( 
    {
        ...
        #	Currently only single gene perturbation analyss is supported
        #	therefore, we ignore the value of --join (=1)
        if (doGPA) {
            cat('\n\n'); banner('GPA')
            # this is arbitrary and should be an option
            #method <- 'fba' 
            #method <- 'lmoma'
            #method <- 'room'
            #method <- 'mtf'
            method <- gpaMethod
            perturbation <- NULL
	    #singleGDA(model, outDir=outDir, verbose=6, method=method)
            if (file.exists(koGenesName)) {
                perturbation <- read.table(koGenesName, stringsAsFactors=FALSE)
            }
            if (is.null(perturbation) || length(perturbation) == 0) {
                cat("Computing Gene Perturbation Analysis for all genes\n")
                singleGDA(model, outDir=outDir, verbose=verbose, method=method)
            } else if (length(perturbation) == 1) {
                gene <- perturbation[1]
                cat("Computing Gene Perturbation Analysis for ", gene,"\n")
                singleGDA(model, gene=gene, 
                    outDir=outDir, verbose=verbose, method=method)
            } else if (lengh(perturbation) == 2) {
                gene <- perturbation[1]
                low.bnd <- perturbation[2]
                cat("Computing Gene Perturbation Analysis for ", gene,
                    "lb =", low.bnd, "\n")
                singleGDA(model, gene=gene, low.bnd=low.bnd, 
                    outDir=outDir, verbose=verbose, method=method)
            } else {
                gene <- perturbation[1]
                low.bnd <- perturbation[2]
                upp.bnd <- perturbation[3]
                cat("Computing Gene Perturbation Analysis for ", gene,
                    "lb =", low.bnd, "ub =", upp.bnd, "\n")
                singleGDA(model, gene=gene, low.bnd=low.bnd, upp.bnd=upp.bnd,
                    outDir=outDir, verbose=verbose, method=method)
            }
	    banner('DONE')
        }
		...
		# we could check if it is a .R file and then, if it is
	        ...
	        dynamicConstraints <- getDynamicConstraints(exchangeRates,
		    timeStep, nSteps)
	    }
            else {
            	dynamicConstraints <- NULL
